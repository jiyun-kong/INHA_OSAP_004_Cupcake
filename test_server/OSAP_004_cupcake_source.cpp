...
void AvlTree::rebalance_subtree(Node *&node_ptr, const int &key) {
  ...
  // LL case
  if (balance_factor > 1 && key < node_ptr->get_left_child()->get_key()) {
    node_ptr = rotate_right(node_ptr);
  }

  // LR case
  else if (balance_factor > 1 && key > node_ptr->get_left_child()->get_key()) {
  ...
  // RL case
           ...
           key < node_ptr->get_right_child()->get_key()) {
